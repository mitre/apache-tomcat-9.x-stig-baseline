control 'V-222961' do
  ...
  Set to true to allow the context (application) to use container servlets,
    ...
    privileged attribute will change the context\'s parent class loader to be
    the Server class loader rather than the Shared class loader.
  '
  desc 'rationale', ''
  ...
  Individual Context elements may be explicitly defined in an individual file
    ...
    context element in the server.xml file as changes will require a server
    restart.

  The $CATALINA_BASE/conf/context element information will be loaded by all
    web applications, the META-INF/context.xml will only be loaded by that
  ...
  grep -i privileged $CATALINA_BASE/webapps/<application name>META-
    INF/context.xml

  If the privileged context attribute is set to true, confirm the application
    has been approved for privileged mode by the ISSO. If the application is
    not approved to run in privileged mode, this is a finding.
  '
  desc 'fix', '
  ...
  If privileged mode is required for a particular application, verify trust
    ...
    applications that are approved to run in privileged mode and retain
    approvals in the system security plan (SSP) for CCRI reviews.
  '
