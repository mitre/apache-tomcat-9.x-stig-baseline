control 'V-222987' do
  ...
  Tomcat file permissions must be restricted. The standard configuration is
    to have Tomcat files contained in the conf/ folder as members of the
    "tomcat" group. While root has read/write privileges, group only has read
    permissions, and world has no permissions. The exceptions are the logs,
    temp, and work directories that are owned by the Tomcat user rather than
    root. This means that even if an attacker compromises the Tomcat process,
    ...
    of 0027 to maintain these permissions. Note that running Tomcat in a Docker
    environment can impact how file permissions and user ownership settings are
    applied. Due to associated Docker configuration complexities, the STIG is
    scoped for standalone rather than virtual Docker deployments.
  ...
  sudo find $CATALINA_BASE/conf -follow -maxdepth 0 \\(  ! -user root -o !
    -group tomcat \\) -ls

  If ISSM risk acceptance specifies deviation from requirement based on
    operational/application needs, this is not a finding if the group
    permissions are set in accordance with the risk acceptance. Ownership must
    not be changed.
  ...
  If operational/application requirements specify different group file
    ...
    risk acceptance.  Ownership must not be changed.

  Run the following commands on the Tomcat server:

  sudo find $CATALINA_BASE/conf -maxdepth 0 \\( ! -user root \\) | sudo xargs
    chown root

  sudo find $CATALINA_BASE/conf -maxdepth 0 \\( ! -group tomcat \\) | sudo
    xargs chgrp tomcat
  '
  ...
  tomcat_conf_dir = file("#{catalina_base}/conf")
