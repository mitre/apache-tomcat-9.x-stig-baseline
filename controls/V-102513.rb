...
uses the JNDIRealm to look up users in an LDAP directory server.  The realm's
connection to the directory is defined by the 'connectionURL' configuration
attribute. This attribute is usually an LDAP URL that specifies the domain name
of the directory server to connect to.
...
files is: $CATALINA_BASE/webapps/<AppName>/WEB-INF/web.xml
...
settings on a case by case basis as per your individual LDAP server and schema.
  ...
  tag gid: 'V-102513'
  tag rid: 'SV-111455r1_rule'
  tag stig_id: 'TCAT-AS-000690'
  tag fix_id: 'F-108047r1_fix'
  tag cci: ['CCI-000197']
  tag nist: ['IA-5 (1) (c)']
