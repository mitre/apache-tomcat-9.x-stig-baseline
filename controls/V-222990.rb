control 'V-222990' do
  ...
  Tomcat\'s file permissions must be restricted. The standard configuration is
    ...
    has read/write privileges, tomcat group only has read and world has no
    permissions. The exceptions are the logs, temp and work directory that are
    owned by the Tomcat user rather than root. This means that even if an
  ...
  sudo find $CATALINA_BASE/temp -follow -maxdepth 0 -type d \\( \\! -perm 750
    \\) -ls
  ...
  sudo find  $CATALINA_BASE/temp -follow -maxdepth 0 -type d -print0 | sudo
    xargs chmod 750 $CATALINA_BASE/temp
  '
