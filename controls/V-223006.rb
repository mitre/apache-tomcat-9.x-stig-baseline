control 'V-223006' do
  ...
  desc '
  Deploying applications to Tomcat requires a Tomcat user account that is in
    the "manager-script" role. Any user accounts in a Tomcat management role
    must be approved by the ISSO.
  '
  desc 'rationale', ''
  ...
  Ensure that user accounts and roles with access to Tomcat management
    features such as the "manager-script" role are documented and approved by
    the ISSO.
